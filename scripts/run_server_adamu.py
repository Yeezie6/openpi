...
# IMPORTANT: Add local lerobot and openpi to path
current_dir = Path(__file__).resolve().parent
workspace_root = current_dir.parent
sys.path.append(str(workspace_root / "lerobot" / "src"))
...
class DexHandDualInferenceServer:
    """
    Minimal implementation of DexHandDualInferenceServer using ZMQ.
    Replaces the missing import from dexhand_dual_service.
    """
    def __init__(self, policy, host, port, api_token=None):
        self.policy = policy
        self.host = host
        self.port = port
        ...
        logger.info(f"Server bound to tcp://{host}:{port}")
    ...
    def run(self):
        while True:
            try:
                ...
                # Support both "data" and "payload" keys, or use the whole message as data if no command
                # But based on endpoints, command is required.
                data = message.get("data", message.get("payload", {}))
                
                if command == "ping":
                    self.socket.send_json({"status": "ok", "message": "pong"})
                
                elif command == "kill":
                    self.socket.send_json({"status": "ok", "message": "shutting down"})
                    break
                
                elif command == "get_action":
                    try:
                        # data should be observations
                        result = self.policy.get_action(data)
                        actions = result["actions"]
                        if isinstance(actions, np.ndarray):
                            actions = actions.tolist()
                        self.socket.send_json({"status": "ok", "actions": actions})
                    except Exception as e:
                        logger.error(f"Error in get_action: {e}")
                        self.socket.send_json({"status": "error", "message": str(e)})
                        
                elif command == "get_modality_config":
                    try:
                        config = self.policy.get_modality_config()
                        self.socket.send_json({"status": "ok", "config": config})
                    except Exception as e:
                        logger.error(f"Error in get_modality_config: {e}")
                        self.socket.send_json({"status": "error", "message": str(e)})
                
                else:
                    self.socket.send_json({"status": "error", "message": f"Unknown command: {command}"})
                    
            except Exception as e:
                logger.error(f"Server loop error: {e}")
                # Try to recover or send error
                try:
                    self.socket.send_json({"status": "error", "message": f"Server internal error: {str(e)}"})
                except:
                    pass
